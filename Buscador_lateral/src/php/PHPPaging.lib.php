...
class PHPPaging {
        ...
        **/
        function agregarArray ($input) {
                if (!is_array($input)) return $this->error(true, "La matriz de datos ingresada no es válida. Recuerde que debe indicar una variable de tipo array.");
        ...
        function error($desarrollo = true, $msg, $query = false, $limit_error = null) {
                if($this->modo != 'publicacion' and $this->mostre_error == false) {
                        if($this->modo == 'reporte') {
                                ...
                                $error.= "Si no logra solucionar el problema, envíe un mensaje a <code>marcos@ocrestudi.es</code> indicando el error mostrado y la consulta que generó el error";
                        } else {
                                $error = "Hubo un error ejecutando la paginación. El mensaje devuelto es: <blockquote><code style='color: #080; font-size: 13px;'><strong>$msg</strong></code></blockquote>Si no logra solucionar el problema, envíe un mensaje a <code>marcos@ocrestudi.es</code> indicando el error mostrado y la consulta que generó el error";
                        }
